...
+(void)requestOrderListWithWaybillnumber:(NSString *)waybillNumber
    ...
    [self getUrl:@"/zhongcheyun/waybill/1/list/shipper/total" delegate:delegate parameters:dic success:success failure:failure];
}
...
+ (void)requestOperateOrder:(double)identytiy
    ...
    ModelAddress * modelAddressItem = [GlobalMethod readModelForKey:LOCAL_LOCATION_UPTODATE modelName:@"ModelAddress"];
...
+(void)requestCarTeamWithEntid:(double)entId
          ...
          carrierEntSimpleName:(NSString *)carrierEntSimpleName
                      ...
                      delegate:(id <RequestDelegate>)delegate
                       ...
                       failure:(void (^)(NSString * errorStr, id mark))failure{
    NSDictionary *dic = @{@"entId":NSNumber.dou(entId),
                          @"carrierEntName":RequestStrKey(carrierEntName),
                          @"carrierEntSimpleName":RequestStrKey(carrierEntSimpleName),
                          @"page":NSNumber.dou(page),
                          @"count":NSNumber.dou(count)};
    [self getUrl:@"/zhongcheyun/contractent/shipper/list/total" delegate:delegate parameters:dic success:success failure:failure];
}
...
+(void)requestCarWithEntid:(double)entId
             ...
             vehicleNumber:(NSString *)vehicleNumber
                  ...
                  delegate:(id <RequestDelegate>)delegate
                   ...
                   failure:(void (^)(NSString * errorStr, id mark))failure{
    NSDictionary *dic = @{@"entId":NSNumber.dou(entId),
                          @"contractEntId":NSNumber.dou(contractEntId),
                          @"driverName":RequestStrKey(driverName),
                          @"vehicleNumber":RequestStrKey(vehicleNumber),
                          @"page":NSNumber.dou(page),
                          @"count":NSNumber.dou(count)};
    [self getUrl:@"/zhongcheyun/vehicle/list/1_0_30/pass" delegate:delegate parameters:dic success:success failure:failure];
}
                       ...
                       paymentChannel:(double)paymentChannel
                        paymentMethod:(double)paymentMethod
                             ...
                             delegate:(id <RequestDelegate>)delegate
                              success:(void (^)(NSDictionary * response, id mark))success
                              failure:(void (^)(NSString * errorStr, id mark))failure{
    NSDictionary *dic = @{@"endTime":NSNumber.lon(endTime),
                          @"endTownId":NSNumber.lon(endTownId),
                          @"endAddr":RequestStrKey(endAddr),
                          @"price":NSNumber.dou(price),
                          @"startAddr":RequestStrKey(startAddr),
                          @"shipperPhone":RequestStrKey(shipperPhone),
                          @"endPhone":RequestStrKey(endPhone),
                          @"startTime":NSNumber.lon(startTime),
                          @"cargoName":RequestStrKey(cargoName),
                          @"startContact":RequestStrKey(startContact),
                          @"description":RequestStrKey(description),
                          @"startTownId":NSNumber.lon(startTownId),
                          @"startPhone":RequestStrKey(startPhone),
                          @"endContact":RequestStrKey(endContact),
                          @"fleetPhone":RequestStrKey(fleetPhone),
                          @"actualLoad":NSNumber.dou(actualLoad),
                          @"driverPhone":RequestStrKey(driverPhone),
                          @"loadUnit":RequestStrKey(loadUnit),
                          @"vehicleId":NSNumber.lon(vehicleId),
                          @"entId":NSNumber.lon(entId),
                          @"endEntName":RequestStrKey(endEntName),
                          @"paymentChannel":NSNumber.lon(paymentChannel),
                          @"paymentMethod":NSNumber.lon(paymentMethod),
