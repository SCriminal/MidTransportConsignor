...
- (void)resetViewWithAry:(NSArray *)ary  modelOrder:(ModelOrderList *)modelOrder{
    ...
    for (ModelOrderOperateTimeItem * item in ary) {
        ModelBtn * model = [ModelBtn new];
        model.title = [ModelOrderList transformOrderStatusToName:item.formState orderType:modelOrder.orderType];
        strMax = strMax.length>model.title.length?strMax:model.title;
        model.subTitle = [GlobalMethod exchangeTimeWithStamp:item.stateTime andFormatter:TIME_SEC_SHOW];
        model.isSelected = true;
        [aryModels addObject:model];
    }
    CGFloat widthMax = [GlobalMethod fetchWidthFromLabel:^(){
        UILabel * label = [UILabel new];
        label.fontNum = F(15);
        label.text = strMax;
        return label;
    ...
    for (int i = 0; i<aryModels.count; i++) {
        ...
        viewDot.backgroundColor = model.isSelected?COLOR_ORANGE:COLOR_666;
        viewDot.leftCenterY = XY(label.left+ widthMax + W(20), label.centerY);
...
- (void)resetViewWithModel:(ModelOrderList *)model{
    ...
    viewDotRed.backgroundColor = COLOR_ORANGE;
...
- (void)resetViewWithModel:(ModelOrderList *)model{
    ...
    self.ivHead.image = [UIImage imageNamed:IMAGE_HEAD_DEFAULT];
    ...
    [self.labelPhone fitTitle:UnPackStr([GlobalData sharedInstance].GB_UserModel.account) variable:0];
